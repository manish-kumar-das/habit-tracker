...
class CompleteHabitHubUI(QWidget):
    ...
    def setup_ui(self):
        ...
        habits_scroll.setMinimumHeight(300)
        ...
        habits_container.setStyleSheet("background-color: transparent;")
        self.habits_list = QVBoxLayout(habits_container)
        self.habits_list.setSpacing(8)
        self.habits_list.setContentsMargins(0, 0, 8, 0)

        habits_scroll.setWidget(habits_container)
        today_layout.addWidget(habits_scroll, stretch=1)
...
class DashboardContent(QWidget):
    ...
    def load_data(self):
        ...
        self.streak_label.setText(f"{max_streak} Day Streak!")
# Replace the existing DashboardContent class with this complete version:

class DashboardContent(QWidget):
    """Dashboard content without sidebar - FIXED VERSION"""
    ...
    def setup_ui(self):
        ...
        # Today's habits - NO VIEW ALL BUTTON
        today_card = QFrame()
        today_card.setStyleSheet("QFrame { background-color: #FFFFFF; border-radius: 20px; }")
        ...
        # Just title, no button
        today_title = QLabel("Today's Habits")
        today_title.setFont(QFont("SF Pro Display", 20, QFont.Bold))
        today_title.setStyleSheet("color: #212529;")
        today_layout.addWidget(today_title)
        
        # Scrollable area for ALL habits
        habits_scroll = QScrollArea()
        habits_scroll.setWidgetResizable(True)
        habits_scroll.setFrameShape(QFrame.NoFrame)
        habits_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        ...
        habits_scroll.setWidget(habits_container)
        today_layout.addWidget(habits_scroll)
