        ...
        try:
            if checked:
                ...
                if notes_dialog.exec():
                    ...
                    success = self.habit_service.mark_habit_complete(
                        self.habit.id, notes=notes
                    )
                    ...
                    if success:
                        ...
                        try:
                            ...
                            streak_info = self.streak_service.get_streak_info(
                                self.habit.id
                            )
                            notif_service.send_streak_milestone(
                                self.habit.name, streak_info["current_streak"]
                            )
    ...
    def apply_filter_and_sort(self):
        ...
        if self.current_filter:
            self.filtered_habits = [
                ...
                if self.current_filter in h.name.lower()
                or (h.description and self.current_filter in h.description.lower())
                or self.current_filter in h.category.lower()
            ]
        else:
        ...
        if self.current_sort == "name_asc":
            ...
            self.filtered_habits.sort(
                key=lambda h: self.streak_service.get_streak_info(h.id)[
                    "current_streak"
                ],
                reverse=True,
            )
    ...
    def display_habits(self):
        ...
        if not self.filtered_habits:
            ...
            text_label.setStyleSheet(
                "color: #9AA0A6; background: transparent; margin-top: 16px;"
            )
            ...
            subtext.setStyleSheet(
                "color: #6B6E76; background: transparent; margin-top: 8px;"
            )
            ...
            for habit in self.filtered_habits:
                ...
                self.habits_layout.insertWidget(
                    self.habits_layout.count() - 1, habit_item
                )
