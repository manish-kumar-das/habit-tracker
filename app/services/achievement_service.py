...
class AchievementService:
    ...
    def check_and_unlock_achievements(self):
        ...
        # Check goals
        goals = goal_service.get_all_goals(include_completed=False)
        if len(goals) > 0 and self.unlock_achievement('goal_setter'):
            newly_unlocked.append('Goal Setter')
        
        completed_goals = goal_service.get_completed_goals_count()
        if completed_goals > 0 and self.unlock_achievement('goal_achiever'):
            newly_unlocked.append('Goal Achiever')
        
        # Check perfect week/month (all habits completed)
        if habits:
            ...
            for i in range(7):
                ...
                for habit in habits:
                    if not habit_service.is_habit_completed_on_date(habit.id, date):
                        perfect_week = False
                        break
                if not perfect_week:
                    break
            
            if perfect_week and self.unlock_achievement('perfect_week'):
                newly_unlocked.append('Perfect Week')
            
            # Check last 30 days
            if perfect_week:  # Only check month if week is perfect
                perfect_month = True
                for i in range(30):
                    date = (datetime.now() - timedelta(days=i)).strftime("%Y-%m-%d")
                    for habit in habits:
                        if not habit_service.is_habit_completed_on_date(habit.id, date):
                            perfect_month = False
                            break
                    if not perfect_month:
                        break
                
                if perfect_month and self.unlock_achievement('perfect_month'):
                    newly_unlocked.append('Perfect Month')
...
# Add method to habit_service to check date completion
def is_habit_completed_on_date(habit_id, date_str):
    """Check if habit was completed on specific date"""
    from app.services.habit_service import get_habit_service
    habit_service = get_habit_service()
    completions = habit_service.get_habit_completions(habit_id)
    return date_str in completions
